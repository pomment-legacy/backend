...
const checkUpdate = require('../upgrade/check');
...
const testSMTP = async () => {
    if (await whiptail.yesno(`Send a test email to ${config.siteAdmin.email}?`)) {
        try {
            if (config.guestNotify.mode === 2) {
                const mailProvider = safeRequire(path.resolve(computedPath, 'mail_provider'));
                if (mailProvider === null) {
                    await whiptail.msgbox('Failed to require the mail provider module. Thus, your mail will not be sent.');
                    return;
                }
                if (mailProvider.forVersion !== 2) {
                    await whiptail.msgbox('Wrong target version of mail provider. Thus, your mail will not be sent.');
                    return;
                }
                ...
                await whiptail.msgbox('Message sent via custom mail service provider');
                return;
            }
...
const setNotificationType = async () => {
    ...
    if (result >= 1 && result <= 3) {
        if (result === 3) {
            ...
            } else if (requireTest.forVersion !== 2) {
                await whiptail.msgbox('Your custom mail provider module\'s target version is wrong. Please re-check before enabling.');
            } else {
...
const securityMenu = async () => {
    const result = Number(await whiptail.menu('Choose an item you want to do', {
        ...
        2: 'Set reCAPTCHA v3 secret key',
        3: 'Enable custom anti-spam provider',
        5: '================================',
        6: 'Go Back',
    }));
    switch (result) {
        ...
        case 3: {
            const option = await whiptail.yesno('Enable custom anti-spam provider?');
            if (!option) {
                config.antiSpamProvider = false;
                break;
            }
            const requireTest = safeRequire(path.resolve(computedPath, 'antispam_provider'));
            if (requireTest === null) {
                await whiptail.msgbox('Your custom anti-spam module is not ready. Please re-check before enabling.');
            } else if (requireTest.forVersion !== 2) {
                await whiptail.msgbox('Your custom anti-spam module\'s target version is wrong. Please re-check before enabling.');
            } else {
                config.antiSpamProvider = true;
            }
            updateConfig();
            break;
        }
