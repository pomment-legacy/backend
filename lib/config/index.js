...
const whiptail = new Whiptail({ notags: true, program: process.platform === 'linux' ? 'whiptail' : 'dialog' });
let config;
let computedPath;
...
const securityMenu = async () => {
    ...
    switch (result) {
        ...
        case 2: {
            config.reCAPTCHA.siteKey = await whiptail.inputbox(
                'Input your reCAPTCHA v3 site key', config.reCAPTCHA.siteKey,
            );
            break;
        }
        case 3: {
            config.reCAPTCHA.secretKey = await whiptail.inputbox(
                'Input your reCAPTCHA v3 secret key', config.reCAPTCHA.secretKey,
            );
            break;
        }
        case 4: {
