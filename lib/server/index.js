...
const checkSubmit = require('./check_submit');
const getUnsubPage = require('./unsubscribe_page');
...
module.exports = (userPath = process.cwd()) => {
    ...
    const globalContext = { $POC, $POD, $POM };

    const auth = hash => hash === $POC.apiKey;
    ...
    koaRouter.post('/v2/submit', async (ctx) => {
        ...
        setTimeout(async () => {
            ...
            if ($POC.reCAPTCHA.enabled) {
                ...
                const result = await reCAPTCHA(globalContext, $POC.reCAPTCHA.secretKey, body.responseKey, logger);
            ...
            if ($POC.antiSpamProvider) {
                logger.info('Verifying user request (Custom)');
                const myProvider = safeRequire(path.resolve($POM.path, 'antispam_provider'));
                if (myProvider === null) {
                    logger.warn('Your custom anti-spam module is not ready. Please re-check before enabling.');
                } else if (myProvider.forVersion !== 2) {
                    logger.warn('Your custom anti-spam module\'s target version is wrong. Please re-check before enabling.');
                } else {
                    const answer = myProvider.verify(query);
                    if (!answer) {
                        await $POD.editPost(body.url, query.id, {
                            hidden: true,
                        }, {
    ...
    koaRouter.post('/unsubscribe/:thread/:id/:editKey', async (ctx) => {
    ...
    koaRouter.get('/unsubscribe/:thread/:id/:editKey', async (ctx) => {
        ...
        try {
            ...
            ctx.response.body = getUnsubPage(url, wanted.content);
        } catch (e) {
            logger.error(e.toString());
            ctx.status = 500;
        }
    ...
    app.use((ctx, next) => {
        if (argv['add-header']) {
            masterLogger.warn('--add-header is discouraged. You should using a reverse-proxy server to handle that.');
    ...
    app.proxy = !argv['add-header'];

    masterLogger.level = $POM.logLevel;
    masterLogger.info(`The HTTP server is http://${$POC.apiHost}:${$POC.apiPort}`);

    const toMB = e => Math.round(e / 1024 / 1024 * 100) / 100;
    if (process.env.NODE_ENV === 'development') {
        setInterval(() => {
            const status = process.memoryUsage();
            masterLogger.debug(`external:  ${toMB(status.external)} MiB`);
            masterLogger.debug(`heapTotal: ${toMB(status.heapTotal)} MiB`);
            masterLogger.debug(`heapUsed:  ${toMB(status.heapUsed)} MiB`);
            masterLogger.debug(`rss:       ${toMB(status.rss)} MiB`);
        }, 10000);
    }
};
