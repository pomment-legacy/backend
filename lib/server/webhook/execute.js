...
const executeWebhook = async (globalContext, data, logger) => {
    const { $POM } = globalContext;
    const webhookString = fs.readFileSync(path.join($POM.path, 'webhooks.txt'), { encoding: 'utf8' })
        .trim();
    if (webhookString === '') {
        logger.debug('No webhook entry found');
        return false;
    }
    const webhookList = webhookString.split('\n');
    logger.debug(webhookList);
