...
class PommentData {
    ...
    /**
     * 获得某个评论串的标题
     * @returns {Promise<string>} 评论串的标题
     */
    async getThreadTitle(url) {
        if (!this._ormThread) {
            this.initORMThread();
        }
        const tmpContent = await this._ormThread.find({
            where: {
                url,
            },
        }).catch((e) => {
        ...
        return tmpContent.dataValues.title;
    }
     ...
     */
    async addThreadTitle(url, title) {
        if (!this._ormThread) {
            this.initORMThread();
        }
        const verifyExist = await this._ormThread.find({
            where: {
                url,
            },
        }).catch((e) => {
        ...
        if (verifyExist) {
            return verifyExist.dataValues;
        }
        const result = await this._ormThread.create({
            url,
            title,
        }).catch((e) => {
            throw e;
